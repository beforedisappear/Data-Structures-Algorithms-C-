...
> определяются по формуле F~i~ = F~i-1~ + F~i-2~, где i ≥ 3. То есть:
> F~1~ = 1, F~2~ = 1, F~3~ = 2, F~4~ = 3 и т.д.
>
...
> Изначально заданы: F~1~ = 1, F~2~ = 1, в отличии от рекурсивной
> версии.
>
...
1)  Метод половинного деления для нахождения корней уравнения
    (Метод PolDel)
...
1)  Поиск минимума функции на отрезке методом золотого сечения
    (Метод GoldMin)
...
<span id="_Hlk104577661" class="anchor"></span>Лабораторная работа № 2
...
Для выполнения данной лабораторной работы мной был реализован обобщенный
...
-   public T\[\]data; (массив данных для формирования
    конечного множества)

Класс Set&lt;T&gt; содержит следующие методы и свойства:

1.  Public bool Contains (T el) – проверка на наличие элемента в
    множестве

> *Входные данные:* T el (элемент, который проверяем)
>
> *Выходные данные:* True/False

1.  Public int GetIndex (T el) – получение индекса элемента множества

> *Входные данные:* T el (элемент, индекс которого ищем)

*Выходные данные:* i (индекс искомого элемента)

1.  public void Add (T el) – добавление нового элемента в множество

> *Входные данные:* T el (элемент, который хотим добавить)
>
> \*добавление возможно в случае отсутствия данного элемента в
> множестве!

1.  public void Resize (int newsize) – изменение размера множества

> *Входные данные:* int newsize (размер нового множества)

1.  public bool RemoveInd (int index) – удаление элемента по индексу

*Входные данные:* T el (индекс удаляемого элемента)

*Выходные данные:* True / False

1.  public bool RemoveEL (T el) – удаление элемента по значению

> *Входные данные:* T el (значение удаляемого элемента)

*Выходные данные:* True / False

1.  public T GetElementByIndex (int i) – получение элемента множества по
    индексу

> *Входные данные:* int i (индекс элемента)
>
> *Выходные данные:* data\[i\] (элемент множества под i-ом индексом)

1.  public T SetElementByIndex (int index, T newEl) – установка нового
    элемента по индексу

> *Входные данные:* int index, T newEl (индекс, по которому происходит
> замена и новое значение элемента)
>
> *Выходные данные:* data\[index\] (измененный элемент)

1.  public static Set&lt;T&gt; Union (Set&lt;T&gt; s1, Set&lt;T&gt; s2)
    – объединение множеств

> *Входные данные:* Set&lt;T&gt; s1, Set&lt;T&gt; s2 (первое и второе
> множество)
>
> *Выходные данные:* Set&lt;T&gt; rab (объединенное множество)

1.  public static Set&lt;T&gt; Intersection (Set&lt;T&gt; s1,
    Set&lt;T&gt; s2) – пересечение множеств

> *Входные данные:* Set&lt;T&gt; s1, Set&lt;T&gt; s2 (первое и второе
> множество)
>
> *Выходные данные:* Set&lt;T&gt; rab (пересеченное множество)

1.  public static Set&lt;T&gt; Addition (Set&lt;T&gt; s1,
    Set&lt;T&gt; s2) – дополнение множества

> *Входные данные:* Set&lt;T&gt; s1, Set&lt;T&gt; s2 (первое и второе
> множество)
>
> *Выходные данные:* Set&lt;T&gt; rab (дополненное множество)

1.  public List&lt;Set&lt;T&gt;&gt; SelectSets() – получение набора
    множеств из последовательности элементов множества

*Входные данные:* Set&lt;T&gt; strs (исходное множество)

*Выходные данные:* List&lt;Set&lt;T&gt;&gt; lsname (список все наборов)

1.  public void Allsubsets(T\[\] list) – поиск всех подмножеств
    множества

*Входные данные:* T\[\] mas1 (массив исходного множества)

*Выходные данные:* string\[\] result\[i\] (i-ое подмножество)

1.  public override string ToString() <span id="_Hlk104578158"
    class="anchor"></span>– преобразование object в строковое
    представление
...
![](media/image1.png){width="2.4862390638670164in"
...
![](media/image2.png){width="2.645968941382327in"
height="0.34029527559055117in"}

*Дополнением* множества А до множества B называется совокупность
элементов множества B, которые не входят в множество A.
...
возрастанию или убыванию. При сортировке элементов в массиве выполняются
две основных операции:\
сравнения элементов по ключу сортировки и пересылка элементов. Для
выполнения данной лабораторной работы мной был реализован класс MySort.
В нем было реализовано множество различных методов (сортировок), где
параметр типа «T» реализует интерфейс «Icomparable».

В классе MySort представлены следующие методы:

-   public static void BubleSort&lt;T&gt;(T\[\] arr) – Пузырьковая
    сортировка

Свое образное название этот метод получил в силу одной своей
особенности:\
...
значений в неотсортированной части массива. Соответственно, при
сортировке по убыванию «всплывают» минимальные значения. Происходит
последовательное сравнение значений соседних элементов и смена чисел
...
В сортировке методом пузырька количество итераций внешнего цикла
определяется длинной массива минус единица, так как когда второй элемент
становится на свое место, то первый уже однозначно минимальный и
находится на своем месте. То есть за первую итерацию максимальное число
становится на свое место.

-   public static void SelectionSort&lt;T&gt;(T\[\] arr) – Сортировка
    выбором
...
После этого массив можно рассматривать как состоящий из двух частей:
левой – уже отсортированной, и правой – неотсортированной. Повторное
применение шагов 1 и 2, но уже к неотсортированной части массива
приведет к ее уменьшению на один элемент и, соответственно, к увеличению
на один элемент отсортированной части массива. Алгоритм повторяется до
тех пор, пока массив не будет отсортирован (размерность
неотсортированной части массива равна единице).

-   public static void InsertionSort&lt;T&gt;(T\[\] arr) – Сортировка
    вставкой

В этом варианте алгоритма включение нового элемента в отсортированную
часть осуществляется путем последовательного сдвига данного элемента
влево. Этот процесс может завершиться при выполнении одного из двух
условий:
...
![](media/image3.png){width="3.1597222222222223in"
height="2.250761154855643in"}Слева продемонстрирована работа данного
алгоритма на последнем шаге сортировки. Выполнилось условие, что
следующий элемент (4) меньше, чем включаемый (7) и произошел выход из
цикла. Далее “указатель” достигает правой границы массива, что означает
завершение процесса сортировки.

-   public static void ShakerSort&lt;T&gt;(T\[\] arr) – Шейкер
...
-   public static void ShellSort&lt;T&gt;(T\[\] arr) – Сортировка Шелла
...
-   public static int BinSearch&lt;T&gt;(T\[\] arr, T el) – бинарный
    поиск

*Входные данные:* T\[\] arr (отсортированный массив), T el (значение
искомого элемента).

*Выходные данные:* int mid (индекс искомого элемента).
...
Максимальное число требующихся сравнений равно log~2~N.

-   public static T\[\] QuickSort&lt;T&gt;(T\[\] arr, int minindex,
    int maxindex) – Быстрая сортировка

> *Принимаемые значения:* T\[\] arr (массив), int minindex и int
> maxindex (минимальный и максимальный индекс массив, в пределах которых
> осуществляется сортировка).

-   public static int Partition&lt;T&gt;(T\[\] arr, int minindex,
    int maxindex) – метод для поиска разделяемого элемента в быстрой
    сортировке (вспомогательный метод).

Быстрая сортировка, подобно сортировке слиянием, применяет принцип
"разделяй и властвуй". Ниже описан процесс сортировки массива А\[р..
r\].

*Разделение*. Массив А\[р.. r\] разбивается на два (возможно, пустых)
подмассива A\[p..q-1\] и A\[q+1.. r\], таких, что каждый элемент
А\[p..q-1\] меньше или равен А\[q\], который, в свою очередь, не
превышает любой элемент подмассива A\[q+1.. r\]. Индекс q вычисляется в
ходе процедуры разбиения.

*Властвование*. Подмассивы A\[p..q-1\] и A\[q+1.. r\] сортируются с
...
*Разбиение массива*. Ключевой частью рассматриваемого алгоритма
...
На схеме №1 продемонстрирован процесс данной сортировки на примере
массива из 8 элементов.

![](media/image4.jpeg){width="4.159878608923885in"
height="4.3469389763779525in"}

-   public static T\[\] MergeSort&lt;T&gt;(T\[\] arr, int minindex,
    int maxindex) – сортировка слиянием

> *Принимаемые значения:* T\[\] arr (массив), int minindex и int
> maxindex (минимальный и максимальный индекс массив, в пределах которых
> осуществляется сортировка).

-   public static void Merge&lt;T&gt;(T\[\] arr, int minindex, int
    middleindex, int maxindex) <span id="_Hlk104770516"
    class="anchor"></span>– метод для слияния массивов
...
> ![](media/image5.png){width="4.108760936132984in"
> height="5.549504593175853in"}
...
![](media/image6.png){width="5.19251312335958in"
height="0.9843241469816273in"}
...
-   public SingleNode&lt;T&gt; Next { get; set; } – ссылка на
    следующий узел.
...
1)  class SingleList&lt;T&gt; - обобщенный класс для создания методов
    односвязного списка.

Класс принимает следующие значения и свойства:

-   public int Count { get; set; } – свойство, определяющее количество
    узлов (элементов) в списке.

-   SingleNode&lt;T&gt; head; - ссылка на головной (первый) узел

И методы:

-   public void AddHead(int k, T v) – добавление узла в начало
    (обновление head).

> *Принимаемые значения:* int k (ключ нового узла), T v (значение нового
> узла)
>
> Ниже продемонстрирован процесс добавления нового узла в начало
> односвязного списка.
>
> ![](media/image7.png){width="3.918591426071741in"
> height="1.2612248468941383in"}

-   public void AddEnd(int k, T v) – добавление узла в конец (обновление
    Next у последнего узла).

> *Принимаемые значения:* int k (ключ нового узла), T v (значение нового
> узла)
>
> Ниже продемонстрирован процесс удаления нового узла в начало
> односвязного списка.
>
> ![](media/image8.png){width="4.70588145231846in"
> height="1.4723370516185477in"}

-   public void RemoveHead() – удаление головного (первого) узла

![](media/image9.png){width="3.48128280839895in"
height="1.2150021872265966in"}

-   public void RemoveEnd() – удаление последнего узла

> ![](media/image10.png){width="3.299465223097113in"
> height="1.2606911636045495in"}

-   public bool IsContainsByValue(T value) – проверка на наличие в
    списке узла с определенным значением.

> *Принимаемые значения:* T value (значение проверяемого узла).

Все методы поиска основаны на переборе всех узлов списка с начала.

-   public SingleNode&lt;T&gt; FindByValue(T value) – поиск узла
    по значению.

-   public SingleNode&lt;T&gt; FindByIndex(int index) – поиск узла по
    индексу (0,1,2,3…).

-   public SingleNode&lt;T&gt; FindByKey(int key) – поиск узла по ключу.

-   public void InsertByAfterValue(T select, int k, T v) – вставка
    нового узла после выбранного узла.

> *Принимаемые значения:* T select (значение узла, после которого
> происходит вставка), int k (ключ вставляемого узла), T v (значение
> вставляемого узла).
>
> ![](media/image11.png){width="2.6149792213473315in"
> height="1.9603958880139982in"}

-   public void InsertByBeforeValue(T select, int k, T v) – вставка
    нового узла перед выбранным узлом.

*Принимаемые значения:* T select (значение узла, перед которым
происходит вставка), int k (ключ вставляемого узла), T v (значение
вставляемого узла).
...
*Принимаемые значения:* T value (значение удаляемого узла)

-   public bool RemoveByIndex(int index)

*Принимаемые значения:* T value (индекс удаляемого узла)

-   public bool RemoveByKey(int key)

*Принимаемые значения:* T value (ключ удаляемого узла)
...
![](media/image12.png){width="3.9715649606299213in"
height="1.0057808398950132in"}

-   public void View() – метод для вывода данных об узлах (значение
    и ключ). Благодаря последовательному выводу узлов всего списка (от
...
![](media/image13.png){width="6.271676509186352in"
height="0.7310761154855643in"}

Для реализации двусвязного списка мной были созданы следующие классы:

1)  class DoubleNode&lt;T&gt; – обобщенный класс для создания данных
    об узле.
...
-   public DoubleNode&lt;T&gt; Next { get; set; } – ссылка на
    следующий узел.

-   public DoubleNode&lt;T&gt; Prev { get; set; } – ссылка на
    предыдущий узел.
...
1)  class DoubleList&lt;T&gt; – обобщенный класс для создания методов
    двусвязного списка.

Класс принимает следующие значения и свойства:

-   public int Count { get; set; } – количество узлов(элементов)
    в списке.
...
-   public void AddEnd(int k, T v) - добавление узла в конец
    (обновление tail).

> *Принимаемые значения:* int k (ключ нового узла), T v (значение нового
> узла)

![](media/image15.png){width="5.12373031496063in"
height="2.05080271216098in"}
...
![](media/image16.png){width="4.460416666666666in"
...
![](media/image17.png){width="5.090428696412949in"
height="0.9603958880139982in"}

Все методы поиска основаны на переборе всех узлов списка с начала.

-   public DoubleNode&lt;T&gt; FindByValue(T value) – поиск узла
    по значению.

-   public DoubleNode&lt;T&gt; FindByKey(int key) – поиск узла по ключу.

-   public DoubleNode&lt;T&gt; FindByIndex(int index) – поиск узла по
    индексу (0,1,2,3…).
...
> вставляемого узла).
>
> ![](media/image18.png){width="5.623611111111111in"
...
-   public void ViewForward() – метод для вывода данных об узлах
    (значение и ключ) при проходе по списку от начала до конца
    (отображение двусвязного списка).

<!-- -->

-   public void ViewBack() – метод для вывода данных об узлах (значение
    и ключ) при проходе по списку от конца к началу (отображение
    двусвязного списка).
...
-   public SingleNode&lt;T&gt;\[\] htArray; – массив, элемент которого
    создан на основе узла односвязного списка (в нем содержатся
    все значения).

-   public int\[\] active; – массив для активных (занятых) ячеек.

-   public int Size; – размер хеш-таблицы (максимальное возможное
    количество ячеек).
...
-   public int HashCode(int key) – метод, возвращающий хеш-функцию
    (создает индекс).

> *Принимаемое значение*: int key (ключ нового элемента).
>
> *Возвращаемое значение*: int index (индекс нового элемента, полученный
> с помощью хеш-функции).

-   public void Add(int key, T value) – добавление нового элемента в
    свободную ячейку.

-   public SingleNode&lt;T&gt; SearchByKey(int key) – поиска элемента
    по ключу.

> *Принимаемое значение*: int key (ключ элемента, который мы ищем).
>
> *Возвращаемое значение*: элемент ячейки хеш-таблицы (ключ-значение).

-   public int SearchIndex(int key) – поиск индекса ячейки по ключу.

> *Принимаемое значение*: int key (ключ элемента, индекс которого мы
> ищем).

*Возвращаемое значение*: int j (индекс ячейки, в которой лежит искомый
элемент).

-   public void RemoveByKey(int key) – удаление элемента из ячейки
    по ключу.

-   public void Resize(int newsize) – изменение размера хеш-таблицы.

-   public void View() – метод для отображения хеш-таблицы (вывод данных
    о ячейках).

1)  HTlist – обобщенный класс для реализации хеш-таблиц на основе
    списков (с использованием метода цепочек).

Класс принимает следующие значения:

-   public List&lt;SingleNode&lt;T&gt;&gt;\[\] htLists; - массив
    списков, элемент которого создан на основе узла односвязного списка
    (в нем содержатся все значения).
...
-   public void Adds(int key, T value) – добавление нового элемента в
    любую ячейку.
...
![](media/image19.png){width="2.5948851706036744in"
height="2.1633661417322836in"}

Для реализации бинарных деревьев мной было создано 2 класса:

1)  BinaryNode&lt;T&gt; - обобщенный класс для создания данных об
    узле дерева.
...
-   Public BinaryNode&lt;T&gt; Right; – ссылка на правый дочерний узел
    (правый потомок/наследник).

-   Public BinaryNode&lt;T&gt; Left; – ссылка на левый дочерний узел
    (левый потомок/наследник).

Класс также содержит два конструктора (пустой и с вышеперечисленными
данными) и метод ToString() для преобразования данных об узлах и их
ссылках в строковое представление.

1)  BinaryTree&lt;T&gt; - обобщенный класс для создания методов
    бинарного дерева.
...
-   public BinaryTree() – инициализация корня дерева, то есть
    создание дерева.

И методы:

-   public void AddNodeInThree(int k, T v) – добавление нового узла
    в дерево.
...
> *Принимаемое значение:* int k (ключ искомого узла).
>
> *Возвращаемое значение:* найденный узел или null.
...
>
> ![](media/image20.png){width="6.138614391951006in"
> height="3.518310367454068in"}

-   public BinaryNode&lt;T&gt; MinNode(BinaryNode&lt;T&gt; tmp) – поиск
    узла c минимальным значением ключа от указанной вершины.

-   public BinaryNode&lt;T&gt; MaxNode(BinaryNode&lt;T&gt; tmp) – поиск
    узла c максимальным значением ключа от указанной вершины.

-   public void ViewAscending(BinaryNode&lt;T&gt; tmp) – отображение
    бинарного дерева по возрастанию ключей.

-   public void ViewDescending(BinaryNode&lt;T&gt; tmp) – отображение
    бинарного дерева по убыванию ключей.

-   public void ViewOrder(BinaryNode&lt;T&gt; tmp) – отображение
    бинарного дерева (обход в прямом порядке, то есть от корня, пока не
    дойдем до листьев).
...
![](media/image21.png){width="5.076388888888889in"
height="2.1805555555555554in"}

Графы обычно представлены двумя способами: матрица смежности и список
смежности.
...
-   public double distance; – расстояние, которое прошли при
    посещении вершины.

-   public List&lt;Edge&gt; AdjEdges; – набор смежных ребер.

-   public ColorVertex color; – цвет вершины (для обхода в глубину
    и ширину).
...
1)  Edge – обобщенный класс для создания данных о ребре.
...
1)  Graph – обобщенный класс для создания методов о графе.
...
-   public List&lt;Edge&gt; AllEdges = new List&lt;Edge&gt;(); – список
    всех ребер графа.

-   public int VertexCount { get { return AllVertexes.Count; } } –
    количество вершин графа.

-   public int EdgeCount { get { return AllEdges.Count; } } – количество
    ребер графа.
...
-   public bool AddEdge(Vertex from, Vertex to) – добавление ребра
    в граф.

-   public void RemoveVertex(Vertex vertex) – удаление вершины в графе.

-   public bool VertexIsContains(Vertex vertexName) – проверка графа на
    наличие вершины.

-   public Vertex FindVertex(Vertex vertexName) – поиск вершины по
    названию вершины.
...
![](media/image22.png){width="6.745283245844269in"
height="3.32040791776028in"}

-   public void DFS(Vertex startVertex) – поиск в глубину (обход графа).

-   public void DFS\_Visit(Vertex u) - вспомогательный метод DFS.

> <span id="_Hlk104768936" class="anchor"></span>При обходе графа в
...
> ![](media/image23.png){width="6.179983595800525in"
...
![](media/image24.png){width="1.3541666666666667in"
height="1.2941294838145232in"}

-   public void PrintWay(Vertex startVertex, Vertex vertex) – метод для
    отображения работы DFS.
...
**Реализация QuickSort и MergeSort**

**неркурсивным способом.**

В лабораторной работе №3 “Сортировки” крайне подробно изложен
рекурсивный алгоритм быстрой сортировки и сортировки слиянием. В
качестве индивидуального задания мне было предложено реализовать данные
алгоритмы нерекурсивным способом. Для выполнения данной лабораторной
работы мной был создан обобщенный класс IndTask, в котором описаны
следующие методы:

-   public static T\[\] QuickSortNR&lt;T&gt;(T\[\] arr, int minindex,
    int maxindex) – нерекурсивная быстрая сортировка.
...
-   public static int partition&lt;T&gt;(T\[\] arr, int minindex,
    int maxindex) – метод для поиска разделяемого элемента(pivot) в
    быстрой сортировке (вспомогательный метод).

Для выполнения нерекурсивной быстрой сортировки
используем [стек](https://neerc.ifmo.ru/wiki/index.php?title=%D0%A1%D1%82%D0%B5%D0%BA),
...
![](media/image25.jpeg){width="3.880952537182852in"
height="3.6767738407699038in"}
...
-   public static T\[\] MergeSortNR&lt;T&gt;(T\[\] arr) – нерекурсивная
    сортировка слиянием (головной метод).

-   public static void MergePass&lt;T&gt;(T\[\] arr, int gap,
    int length) – метод для разбиения массива на части (вспомогательный
    метод №1).

-   public static void Merge&lt;T&gt;(T\[\] arr, int minindex, int
    middleindex, int maxindex) – метод для слияния массивов
...
> ![](media/image26.png){width="3.677214566929134in"
> height="4.916724628171479in"}
>
> Также был реализован вариант с использованием стека.

-   public static T\[\] MergeSortNR2&lt;T&gt;(T\[\] arr) – нерекурсивная
    сортировка слиянием (головной метод).

-   public static T\[\] Merge2&lt;T&gt;(T\[\] left, T\[\] right) – метод
    для слияния массивов (вспомогательный метод).
...
> ![](media/image27.png){width="5.358004155730534in"
> height="3.7441863517060368in"}

![](media/image28.png){width="5.988231627296588in"
height="2.9069772528433946in"}

![](media/image29.png){width="6.490277777777778in"
